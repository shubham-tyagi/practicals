...
}*first,*last,*newtemp,*temp,*temp1,*temp2;
...
void insert_mid()
{
  ...
  if(pos==1)
  {
    ...
    {
      printf("\nwrong position entered do you want to enter again if yes enter y ?\n");
      scanf("%c",&ch);
      if(ch=='y'||ch=='Y')
      goto start;
      else
...
void delete_first()
{
  ...
  count--;
  printf("deleting the first element %d ",first->data);
  temp=first;
  first=first->next;
last->next=first;
first->prev=last;
...
void delete_last()
{
  printf("deleting the last element %d",last->data);
  temp=first;
